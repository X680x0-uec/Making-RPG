...
public class Item : ScriptableObject
{
    ...
    //アイテム使用時のロジック
    public void Use(List<Figure> targets)
    {
        if (targets == null || targets.Count == 0) return;

        foreach (Figure Figure in targets)
        {
            switch (this.effect)
            {
                case Effects.Damage:
                    Figure.TakeDamage(this.param);
                    break;
                case Effects.HPRecover:
                    Figure.currentHP = Mathf.Min(Figure.maxHP, Figure.currentHP + this.param);
                    Debug.Log($"{Figure.charaName}のHPが{this.param}回復した！");
                    break;
                case Effects.MPRecover: // ★追加済み
                    Figure.currentMP = Mathf.Min(Figure.maxMP, Figure.currentMP + this.param);
                    Debug.Log($"{Figure.charaName}のMPが{this.param}回復した！");
                    break;
                case Effects.AttackUp:
                    // 倍率(param)をそのまま渡すことで、デバフも可能
                    Figure.ApplyAttackBoost(this.param, this.duration);
                    break;
                case Effects.DefenseUp:
                    Figure.ApplyDefenseBoost(this.param, this.duration);
                    break;
                ...
                case Effects.SpeedUp:
                    Figure.ApplySpeedBoost(this.param, this.duration);
                    break;
